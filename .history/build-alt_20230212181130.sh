...
# WindowID=$(xwininfo -display :0 -root -all | grep Investigation | grep "\(\"Navigator\" \"firefox\"\)" | awk '{print $1}')
WindowID=$(xwininfo -display :0 -root -all | grep Investigation | grep "Investigaton of decision-making" | awk '{print $1}')
